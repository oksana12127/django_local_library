...
class LoanedBookInstancesByUserListViewTest(TestCase):
    ...
    def setUp(self):
        ...
        test_language = Language.objects.create(langauge_book='English')
...
class AuthorCreateViewTest(TestCase):
    def setUp(self):
        ...
        # для имеющего право юзера создаем автора (только для юзера 2)
        # Создание объекта AUTHORcreate для для пользователя test_user2x,
        # при этом проаеряем, что дата смерти больше даты рождения ????????????? Не совсем понимаю, что такое начальная дата
        # self.test_author2 = Author.objects.create(first_name='John', last_name='Smith', date_of_birth='2019-01-01',
        #                                           date_of_death='2020-01-01', username='testuser2')

        # проверить, имеет ли юзер право на создание автора в шаблоне author_form по признаку permission (то есть, если не имеет правв на страницу, юзера1 выбросит в ошибку
    ...
    def test_uses_correct_template(self):
        ...
        # проверить, перенаправление из отображения в случае успеха в шаблон author_detail.html

    def test_redirects_to_author_create_on_success(self):
        login = self.client.login(username='testuser2', password='12345')

        resp = self.client.post(reverse('author_create'), data={"first_name": 'John', "last_name": 'Smith',
                                                               "date_of_birth": '2019-01-01',
                                                               "date_of_death": '2020-01-01'})
        self.assertEqual(resp.status_code, 200)
        self.assertRedirects(resp, reverse('author-detail'))
